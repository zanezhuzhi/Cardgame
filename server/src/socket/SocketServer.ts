...
export class SocketServer {
  ...
  private bindGameEvents(socket: TypedSocket): void {
    ...
    // 薙魂弃牌响应
    socket.on('game:naginataSoulDiscardResponse' as any, (data: { selectedId: string }, callback?: (result: { success: boolean; error?: string }) => void) => {
      const room = this.roomManager.getPlayerRoom(socket.id);
      if (!room || !room.game) {
        callback?.({ success: false, error: '游戏未开始' });
        return;
      ...
      const result = room.game.handleNaginataSoulDiscardResponse(socket.id, data.selectedId);
