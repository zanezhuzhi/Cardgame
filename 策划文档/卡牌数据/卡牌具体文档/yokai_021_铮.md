...
| 项目 | 状态 |
|:----:|:----:|
| 主动效果 | ✅ 已实现（YokaiEffects.ts + MultiplayerGame.ts） |
| 触发系统 | ✅ 已实现（HarassmentPipeline.ts P20 抵抗处理器，含交互选择） |
| 妨害标签 | ✅ 已定义（EffectClassification 中注册） |
| AI逻辑 | ✅ 默认弃置策略（L1规则） |
| 单元测试 | ✅ 已编写（主动效果4用例 + 触发效果3用例 + 轮入道兼容2用例） |
